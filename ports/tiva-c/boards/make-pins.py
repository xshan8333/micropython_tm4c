#!/usr/bin/env python
"""Generates the pins file for the TIVA-C."""
...
SUPPORTED_AFS = { 'UART': ('TX', 'RX', 'RTS', 'CTS'),
                  ...
                  'I2C': ('SDA', 'SCL'),
                  'CAN': ('TX', 'RX'),
                  'M': ('PWM0','PWM1','PWM2','PWM3','PWM4','PWM5','PWM6','PWM7'),
...
class Pin:
    ...
    def __init__(self, name, port, gpio_bit):
        self.name = name
        self.port = port
    ...
    def print(self):
        ...
        if len(self.afs):
            ...
            for af in self.afs:
                af.print()
            print('};')
            print('pin_obj_t pin_{:4s} = PIN({:6s}, {:6s}, {:3d}, pin_{}_af, {});\n'.format(
                   self.name, self.name, self.port, self.gpio_bit, self.name, len(self.afs)))
        else:
            print('pin_obj_t pin_{:4s} = PIN({:6s}, {:6s}, {:3d}, NULL, 0);\n'.format(
                   self.name, self.name, self.port, self.gpio_bit))
...
class Pins:
    ...
    def parse_af_file(self, filename, port_col, pinname_col, af_start_col):
        with open(filename, 'r') as csvfile:
            rows = csv.reader(csvfile)
            for row in rows:
                try:
                    (port, gpio_bit) = parse_port_pin(row[pinname_col])
                except:
                    continue
                #port = row[port_col]
                pin = Pin(row[pinname_col], port, gpio_bit)
